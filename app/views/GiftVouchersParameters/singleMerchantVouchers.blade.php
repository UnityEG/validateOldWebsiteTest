...
<div class="row">	
    ...
    <div style="margin:20px 0px;" class="col-lg-4 col-md-4 col-sm-6 col-xs-12  voucherbox">
        ...
        <a class="voucher-link" href="{{route('GiftVouchersParameters.show', array($voucher->id))}}">
        ...
        <div style="background-color:#fff;" class="voucherttitle">
            <div class="vouchertext">
                <a class="voucher-link" href="{{route('GiftVouchersParameters.show', array($voucher->id))}}">{{{$voucher->Title}}}</a><br />
...
<!--   
   <ul>
       @foreach($data['merchant_gift_vouchers'] as $voucher)
       <li>
           <a href="{{route('GiftVoucher.create', array('customer_id'=>4, 'gift_vouchers_parameters_id'=>$voucher->id))}}">
               @if(is_object($voucher->userPic))
               <div><img src="{{asset($default_image_path.'/'.$voucher->userPic->pic.'.'.$voucher->userPic->extension)}}"></div>
               @endif
               <h3>{{{$voucher->Title}}}</h3>
               <p>{{$voucher->short_description}}</p>
               @if(is_object($voucher->merchant))
               <p>{{{$voucher->merchant->address1}}}</p>
               @endif
           </a>
           <div class="fb-share-button" data-href="{{secure_url('GiftVouchersParameters/gallery?merchant_id='.$voucher->merchant->id)}}" data-layout="button_count"></div>
       </li>
       @endforeach
   </ul> -->
{{$data['merchant_gift_vouchers']->links()}}
